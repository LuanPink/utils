...
public class PanelTableHDBH extends JPanel {
	private Color color = ColorBackground.colorDark;
	private JScrollPane scrollPane;
	private TableHDBH table;
	private Icon iconFilterWhite = new ImageIcon(PanelTableHDBH.class.getResource("/icon/filter-white.png"));
	private JLabel lblNewLabel;
	private Icon iconFilterGray = new ImageIcon(PanelTableHDBH.class.getResource("/icon/filter-gray.png"));
	...
	PanelCTHDBH panelCTHDBH = new PanelCTHDBH();
	public PanelTableHDBH() {
		setOpaque(false);
		table  = new TableHDBH();
		scrollPane = new JScrollPane(table);
		table.addTableStyle(scrollPane);
		lblNewLabel = new JLabel("Hóa Đơn Thanh Toán");
	...
	public void init() {
		hdbhs = daohdbh.selectAll();
		fillTable();
	}
	
	public void fillTable() {
		table.tableModel.setRowCount(0);
		for (ModelHDBH data : hdbhs) {
			long roundValue = Math.round(data.getTolalPrice());
			table.tableModel.addRow(new Object[] {
					data.getMaHDBH(),data.getNameKH(),data.getPhone(),data.getNameNV(),roundValue,data.getNgayBan()
			});
		}
	}
	...
	public void action() {
		table.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				if(e.getClickCount() == 2) {
					...
					String MaHDBH = hdbhs.get(index).getMaHDBH();
					// show DialogCTHHBH
					dialogShow.showForm(panelCTHDBH);
					panelCTHDBH.init(new ModelHDBHCT(MaHDBH));
					int x = 400;
					int y = 80;
		...
		txtSearch.addCaretListener(new CaretListener() {
			...
			public void caretUpdate(CaretEvent e) {
				if(txtSearch.getText().equals("")) {
					...
					hdbhs = daohdbh.Find(new ModelHDBH(txtSearch.getText()));
					fillTable();
				}
			}
		...
		panelCTHDBH.btnCancel.addActionListener(new ActionListener() {
