...
public class PanelHDDHCT extends JPanel {
	...
	DAOHDDHCT daocthdbh = new DAOHDDHCT();
	ArrayList<ModelHDDHCT> datas = new ArrayList<ModelHDDHCT>();
	public PanelHDDHCT() {
		setOpaque(false);

		JLabel lblHanChi = new JLabel("Hóa Đơn Đổi Hàng Chi Tiết");
	...
	public void init(ModelHDDHCT model) {
		datas = daocthdbh.Find(model);
		fillTable();
	}
	
	public void fillTable() {
		table.tableModel.setRowCount(0);
		for (ModelHDDHCT data : datas) {
			long roundValue = Math.round(data.getTotalPrice());
			table.tableModel.addRow(new Object[] {
					data.getMaHDDH(),data.getMaSP(),data.getSize(),data.getColor(),data.getMaterial(),data.getQuantity(),roundValue
			});
		}
	}
