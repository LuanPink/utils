...
public class PanelTableHDDH extends JPanel {
	...
	PanelHDDHCT panelCTHDTH = new PanelHDDHCT();
	...
	public void init() {
		hddhs = daohddh.selectAll();
		fillTable();
	}
	
	public void fillTable() {
		table.tableModel.setRowCount(0);
		for (ModelHDDH data : hddhs) {
			long roundValue = Math.round(data.getTotal());
			table.tableModel.addRow(new Object[] {
					data.getMaHDDH(),data.getMaKH(),data.getNgayDoi(),roundValue
			});
		}
	}
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				if(e.getClickCount() == 2) {
					...
					String MaHDDH = hddhs.get(index).getMaHDDH();
					// show DialogCTHHBH
					dialogShow.showForm(panelCTHDTH);
					panelCTHDTH.init(new ModelHDDHCT(MaHDDH));
					int x = 400;
					int y = 80;
		...
		txtSearch.addCaretListener(new CaretListener() {
			...
			public void caretUpdate(CaretEvent e) {
				if(txtSearch.getText().equals("")) {
					...
					hddhs = daohddh.Find(new ModelHDDH(txtSearch.getText()));
					fillTable();
				}
			}
