...
public class PanelCTHDTH extends JPanel {
	...
	public void init(ModelHDTHCT model) {
		datas = daocthdbh.Find(new ModelHDTHCT(model.getMaHDTH()));
		fillTable();
	}
	
	public void fillTable() {
		table.tableModel.setRowCount(0);
		for (ModelHDTHCT data : datas) {
			long roundValue = Math.round(data.getTotalPrice());
			table.tableModel.addRow(new Object[] {
					data.getMaHDTH(),data.getTenSP(),data.getTenKH(),data.getSize(),data.getColor(),data.getMaterial(),data.getQuantity(),roundValue
			});
		}
	}
