...
public class ButtonProduct extends JPanel {
	...
	public ButtonProduct(ImageIcon image,String name,String price,int index) {
		this.img = image;
		this.name = name;
		this.price = price;
		this.index = index;
	...
	public void action() {
		btnAdd.addActionListener(new ActionListener() {	
			...
			public void actionPerformed(ActionEvent e) {
				String customerId = "1";
				String productId = PanelHome.datas.get(index).getId().getId();
				int quatity = 1;
				double price = PanelHome.datas.get(index).getProductPrice().getPrice();
				...
				datas = dao.SelectByIdProduct(productId);
				...
				if(datas.size()>0) {
					...
					// insert
					if(dao.insert(new ModelItemCart(null,customerId,null,productId,null,null,null,price,quatity,sdf.format(now)))>0){
						question.setVisible(true);
						question.setLocationRelativeTo(null);
						question.lblTextMessage.setText("Đã thêm sản phẩm vào giỏ hàng");
					}else {
