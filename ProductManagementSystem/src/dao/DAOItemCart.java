...
public class DAOItemCart implements DAOProductList<ModelItemCart, Object> {
	...
	final String SELECT_ALL_SQL = "SELECT * FROM dbo.GIOHANG \r\n"
			...
			+ "INNER JOIN dbo.MauSac ON MauSac.MaMau = SanPham.MaMau\r\n" + "WHERE GIOHANG.MaKH = ? ORDER BY ID DESC";

	final String SELECT_BY_IDPRDUCT = "SELECT * FROM GIOHANG WHERE MASP = ?";
		...
		try {
			while (rs.next()) {
				ModelItemCart data = new ModelItemCart(rs.getString("ID"), rs.getString("MAKH"), rs.getString("HINHSP"),
						rs.getString("MASP"), rs.getString("TENSP"), rs.getString("TENSIZE"), rs.getString("TENMAU"),
	...
	public ArrayList<ModelItemCart> SelectByIdProduct(Object k) {
		ArrayList<ModelItemCart> datas = new ArrayList<ModelItemCart>();
		ResultSet rs = JDBCHelper.executeQuery(SELECT_BY_IDPRDUCT, k);
		try {
			while (rs.next()) {
				ModelItemCart data = new ModelItemCart(rs.getString("ID"), rs.getString("MAKH"), null,
						rs.getString("MASP"), null, null, null, rs.getDouble("GIA"), rs.getInt("SLSP"), rs.getString("NGAYTAO"));
				datas.add(data);
			}
